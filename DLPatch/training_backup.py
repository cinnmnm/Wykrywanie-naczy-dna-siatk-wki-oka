...
from DLModel import DLModel, gcn_pil_to_tensor_transform
    ...
    def _discover_valid_patches(self):
        ...
        for img_idx in self.image_indices:
            ...
            if len(centers) == 0:
                ...
                self.image_class_centers[img_idx] = {0: torch.empty((0, 2), device=self.device, dtype=torch.long),
...
class OptimizedTrainer:
    ...
    def create_data_loaders(self) -> Tuple[DataLoader, DataLoader, DataLoader, 'FastPatchSampler', 'FastPatchSampler', 'FastPatchSampler']:
        ...
        with self.timer("DataLoader creation"):
            ...
            assert self.train_indices is not None, "Train indices must be initialized"
            assert self.val_indices is not None, "Val indices must be initialized"
            assert self.test_indices is not None, "Test indices must be initialized"
            ...
            print(f"Created data loaders - Train: {len(train_dataset)}, "
                  f"Val: {len(val_dataset)}, Test: {len(test_dataset)} samples")
      ...
      def train_epoch(self, train_loader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        assert self.loss_fn is not None, "Loss function must be initialized"
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            # Data is already on GPU from FastPatchSampler
      ...
      def evaluate(self, data_loader: DataLoader) -> Dict[str, float]:
        ...
        # Calculate metrics
        accuracy = Evaluate.accuracy(all_targets, all_preds)
        sensitivity = Evaluate.sensitivity(all_targets, all_preds)
        specificity = Evaluate.specificity(all_targets, all_preds)
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            # Generate new samples for this epoch
...
def main():
    ...
    trainer = OptimizedTrainer("config.yaml")
